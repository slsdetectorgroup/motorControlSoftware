...
ReferencePointsWidget::ReferencePointsWidget(QWidget *parent,
                                             std::string hostname,
                                             MotorWidget *x, MotorWidget *y,
                                             MotorWidget *z,
...
void ReferencePointsWidget::LayoutWindow() { LoadReferencePoints(); }

void ReferencePointsWidget::LoadReferencePoints() {
    std::string result = SendCommand(hostname, 1, "reflist ",
                                     "FwheelWidget::LoadReferencePoints");
    if (result.empty()) {
        return;
    }
    ...
    if (list.size() == 0) {
        Message(CRITICAL, "Could not get reference points list",
                "ReferencePointsWidget::LoadReferencePoints");
        return;
    }
    ...
    // disable none
    QModelIndex index =
        comboReference->model()->index(comboReference->count() - 1, 0);
    QVariant v(0);
    comboReference->model()->setData(index, v, Qt::UserRole - 1);
}

void ReferencePointsWidget::Initialization() {
    connect(comboReference, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetReferencePoint()));
...
void ReferencePointsWidget::GetReferencePoint() {
    disconnect(comboReference, SIGNAL(currentIndexChanged(int)), this,
               SLOT(SetReferencePoint()));
    std::string result = SendCommand(
        hostname, 1, "getref ", "ReferencePointsWidget::GetReferencePoint");
    if (!result.empty()) {
        ...
        if (!found) {
            ...
            oss << "Could not match reference point " << result
                << " to any in combo list.";
            Message(WARNING, oss.str(),
                    "ReferencePointsWidget::GetReferencePoint");
        }
    }
    connect(comboReference, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetReferencePoint()));
...
void ReferencePointsWidget::SetReferencePoint() {
    ...
    oss << "setref " << ref;
    std::string result = SendCommand(
        hostname, 2, oss.str(), "ReferencePointsWidget::SetReferencePoint");
