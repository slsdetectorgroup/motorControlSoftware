...
void TubeWidget::DisplayTubeError() {
  ...
  result =
      SendTubeCommand(hostname, 1, "getemess", "TubeWidget::DisplayTubeError");
  if (result.empty()) {
    return;
  }
  ...
  for (unsigned int i = 0; i < chkShutters.size(); ++i) {
    if (checkedBox == chkShutters[i]) {
      ...
      oss << "shutter " << i + 1 << ' '
          << (chkShutters[i]->isChecked() ? "on" : "off");
      std::string result =
          SendTubeCommand(hostname, 3, oss.str(), "TubeWidget::SetShutters");
      if (result.empty()) {
        GetShutters();
      }
      continue;
...
int TubeWidget::CheckWarmup() {
  ...
  if (errorCode == TUBE_ERROR_WARM_UP || errorCode == TUBE_ERROR_WARM_UP2) {
    ...
    if (Message(QUESTION, oss.str(), "TubeWidget::CheckWarmup", "Warm-up?") !=
        OK) {
      DeclineWarmup();
    } else {
      AcceptWarmup();
...
int TubeWidget::GetWarmupTimeRemaining() {
  ...
  while (result.empty()) {
    ...
    try {
      ...
      FILE_LOG(logERROR) << "Could not get warm up time remaining. "
                         << e.what();
      result = "";
    }
  }
