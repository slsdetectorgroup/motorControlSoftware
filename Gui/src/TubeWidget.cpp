...
void TubeWidget::Initialization() {
    ...
    for (unsigned int i = 0; i < chkShutters.size(); ++i) {
        connect(chkShutters[i], SIGNAL(toggled(bool)), this,
                SLOT(SetShutters()));
    }
    connect(spinVoltage, SIGNAL(valueChanged(int)), this,
            SLOT(SetVoltage(int)));
    connect(spinCurrent, SIGNAL(valueChanged(int)), this,
            SLOT(SetCurrent(int)));
...
void TubeWidget::GetConnectedShutters() {
    std::string result = SendTubeCommand(hostname, 1, "connectedshutters ",
                                         "TubeWidget::GetConnectedShutters");
    if (result.empty()) {
        return;
    }
    ...
    if (list.size() < 4) {
        Message(WARNING, "Could not get connected shutters list",
                "TubeWidget::GetConnectedShutters");
        return;
    }
...
void TubeWidget::DisplayTubeError() {
    ...
    int errorCode = -1;
    std::string result =
        SendTubeCommand(hostname, 1, "geterr", "TubeWidget::DisplayTubeError");
    if (result.empty()) {
        return;
    }
    try {
    ...
    result = SendTubeCommand(hostname, 1, "getemess",
                             "TubeWidget::DisplayTubeError");
    if (result.empty()) {
        return;
    }
...
void TubeWidget::ClearError(bool wait) {
    ...
    // wait required when clearing 109 error (warm up necessary, when user
    // declines)
    if (wait) {
        usleep(1 * 1000 * 1000);
    }
...
void TubeWidget::ClearError() { ClearError(false); }

void TubeWidget::GetHighVoltage() {
    disconnect(pushHighVoltage, SIGNAL(toggled(bool)), this,
               SLOT(SetHighVoltage(bool)));
    std::string result =
        SendTubeCommand(hostname, 1, "gethv ", "TubeWidget::GetHighVoltage");
    if (!result.empty()) {
...
void TubeWidget::SetHighVoltage(bool enable) {
    ...
    std::string result =
        SendTubeCommand(hostname, 2, oss.str(), "TubeWidget::SetHighVoltage");
    if (result.empty()) {
        GetHighVoltage();
        if (CheckWarmup()) {
            return;
...
void TubeWidget::GetShutters() {
    for (unsigned int i = 0; i < chkShutters.size(); ++i) {
        disconnect(chkShutters[i], SIGNAL(toggled(bool)), this,
                   SLOT(SetShutters()));
    }
    std::string result =
        SendTubeCommand(hostname, 1, "getshutters ", "TubeWidget::GetShutters");
    if (!result.empty()) {
        std::istringstream iss(result);
        std::vector<std::string> list =
            std::vector<std::string>(std::istream_iterator<std::string>(iss),
                                     std::istream_iterator<std::string>());
        if (list.size() < 4) {
            Message(WARNING, "Could not get shutters state",
                    "TubeWidget::GetShutters");
        } else {
            for (unsigned int i = 0; i < chkShutters.size(); ++i) {
...
void TubeWidget::SetShutters() {
    ...
    for (unsigned int i = 0; i < chkShutters.size(); ++i) {
        if (checkedBox == chkShutters[i]) {
            ...
            oss << "shutter " << i + 1 << ' '
                << (chkShutters[i]->isChecked() ? "on" : "off");
            std::string result = SendTubeCommand(hostname, 3, oss.str(),
                                                 "TubeWidget::SetShutters");
            if (result.empty()) {
                GetShutters();
            }
            continue;
...
void TubeWidget::GetVoltage() {
    disconnect(spinVoltage, SIGNAL(valueChanged(int)), this,
               SLOT(SetVoltage(int)));
    std::string result =
        SendTubeCommand(hostname, 1, "getv ", "TubeWidget::GetVoltage");
    if (!result.empty()) {
        try {
...
void TubeWidget::SetVoltage(int value) {
    ...
    oss << "setv " << value;
    std::string result =
        SendTubeCommand(hostname, 2, oss.str(), "TubeWidget::SetVoltage");
    if (result.empty()) {
        if (CheckWarmup()) {
            return;
        }
...
void TubeWidget::GetCurrent() {
    disconnect(spinCurrent, SIGNAL(valueChanged(int)), this,
               SLOT(SetCurrent(int)));
    std::string result =
        SendTubeCommand(hostname, 1, "getc ", "TubeWidget::GetCurrent");
    if (!result.empty()) {
        try {
            int value = getInteger(result);
...
void TubeWidget::SetCurrent(int value) {
    ...
    oss << "setc " << value;
    std::string result =
        SendTubeCommand(hostname, 2, oss.str(), "TubeWidget::SetCurrent");
    if (result.empty()) {
        if (CheckWarmup()) {
            return;
        }
...
void TubeWidget::GetActualVoltage() {
    std::string result = SendTubeCommand(hostname, 1, "getactualv ",
                                         "TubeWidget::GetActualVoltage");
    if (!result.empty()) {
        try {
            int value = getInteger(result);
...
void TubeWidget::GetActualCurrent() {
    std::string result = SendTubeCommand(hostname, 1, "getactualc ",
                                         "TubeWidget::GetActualCurrent");
    if (!result.empty()) {
        try {
            int value = getInteger(result);
...
int TubeWidget::CheckWarmup() {
    ...
    if (result.empty()) {
        Message(WARNING, "Could not figure out if warm up required.",
                "TubeWidget::CheckWarmup");
        DisplayTubeError();
        return 0;
    }
    try {
        ...
        Message(WARNING, "Could not warm up. " + std::string(e.what()),
                "TubeWidget::CheckWarmup");
        return 0;
    }
    // no error code
    ...
    // unknown error code
        ...
        Message(WARNING, "Unknown Error. Please check error message field.",
...
void TubeWidget::AcceptWarmup() {
    ...
    oss << "warmup " << voltage;
    std::string result =
        SendTubeCommand(hostname, 2, oss.str(), "TubeWidget::CheckWarmup");
    if (result.empty()) {
    ...
    try {
        ...
        Message(WARNING, "Warm up failed. " + std::string(e.what()),
                "TubeWidget::CheckWarmup");
        return;
    }
    if (errorCode != 0) {
        std::ostringstream oss;
        oss << "Warm up failed.  Error Code: " << errorCode
...
void TubeWidget::DeclineWarmup() {
    ...
    ClearError(true);
    statusBar->showMessage("Updating ...");
    // clear also the initial warm up error (clearing 109, will set errorcode to
    // 70 again)
    ClearError();
    statusBar->showMessage("Updating ...");
    UpdateValues();
...
int TubeWidget::GetWarmupTimeRemaining() {
    ...
    std::string result;
    while (result.empty()) {
        result = SendTubeCommand(hostname, 1, "getwtime",
                                 "TubeWidget::UpdateWarmupTiming");
        try {
...
std::string TubeWidget::SendTubeCommand(std::string hostname, int nCommand,
                                        std::string command,
                                        std::string source) {
    try {
