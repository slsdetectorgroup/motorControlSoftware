...
FluorWidget::FluorWidget(QWidget *parent, std::string name,
...
std::string FluorWidget::GetName() { return name; }

void FluorWidget::UpdateHolderList() {
    disconnect(comboHolder, SIGNAL(currentIndexChanged(int)), this,
               SLOT(SetHolder(int)));
    comboHolder->clear();
    LoadTargetHolders();
    connect(comboHolder, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetHolder(int)));
}
...
void FluorWidget::LoadTargetHolders() {
    std::string result = SendCommand(hostname, 2, "numflist " + name,
                                     "FwheelWidget::LoadTargetHolders");
    if (result.empty()) {
        return;
    }
...
void FluorWidget::Initialization() {
    connect(comboHolder, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetHolder(int)));
    connect(comboTarget, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetTarget(int)));
...
void FluorWidget::GetHolder() {
    disconnect(comboHolder, SIGNAL(currentIndexChanged(int)), this,
               SLOT(SetHolder(int)));
    std::string result =
        SendCommand(hostname, 2, "getholder " + name, "FluorWidget::GetHolder");
    if (!result.empty()) {
        try {
...
void FluorWidget::GetTargetList() {
    disconnect(comboTarget, SIGNAL(currentIndexChanged(int)), this,
               SLOT(SetTarget(int)));
    std::ostringstream oss;
    ...
    if (!result.empty()) {
        ...
        if (list.size() == 0) {
            Message(WARNING, "Could not get target list for this holder",
                    "FluorWidget::GetTargetList");
        } else {
            // clear list first
            energy.clear();
            comboTarget->clear();
            ...
            // disable none
            QModelIndex index =
                comboTarget->model()->index(comboTarget->count() - 1, 0);
            QVariant v(0);
...
void FluorWidget::SetHolder(int index) {
    ...
    std::string result =
        SendCommand(hostname, 3, oss.str(), "FluorWidget::SetHolder");
    if (result.empty()) {
        GetHolder();
    }
    if (!result.empty()) {
...
void FluorWidget::GetTarget() {
    disconnect(comboTarget, SIGNAL(currentIndexChanged(int)), this,
               SLOT(SetTarget(int)));
    std::string result =
        SendCommand(hostname, 2, "getfl " + name, "FluorWidget::GetTarget");
    if (!result.empty()) {
        ...
        if (!found) {
            std::ostringstream oss;
            oss << "Could not match target " << result
                << " to any in combo list for " << name;
...
void FluorWidget::SetTarget(int index) {
    ...
    std::string result =
        SendCommand(hostname, 3, oss.str(), "FluorWidget::SetTarget");
