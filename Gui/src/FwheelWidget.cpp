...
FwheelWidget::FwheelWidget(QWidget *parent, std::string name,
...
void FwheelWidget::LoadAbsorptionValues() {
    std::string result = SendCommand(hostname, 2, "fwvals " + name,
                                     "FwheelWidget::LoadAbsoptionValues");
    if (result.empty()) {
        return;
    }
...
void FwheelWidget::Initialization() {
    connect(comboValue, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetValue()));
}

void FwheelWidget::GetValue() {
    disconnect(comboValue, SIGNAL(currentIndexChanged(int)), this,
               SLOT(SetValue()));
    std::string result =
        SendCommand(hostname, 2, "getfw " + name, "FwheelWidget::GetValue");
    if (!result.empty()) {
        ...
        if (!found) {
            ...
            oss << "Could not match absorption value " << result
...
void FwheelWidget::SetValue() {
    ...
    std::string result =
        SendCommand(hostname, 3, oss.str(), "FwheelWidget::SetValue");
...
void FwheelWidget::Update() { GetValue(); }
