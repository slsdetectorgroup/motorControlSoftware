...
void OptionsWidget::Initialization() {
    ...
    connect(comboMotor, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetMotor()));
    ...
    connect(comboReferencePoint, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetRefPoint()));
    connect(comboFluor, SIGNAL(currentIndexChanged(int)), this,
            SLOT(SetFluor()));
...
void OptionsWidget::SetPosition(double value) {
    if (Message(QUESTION,
                "Are you sure you want to overwrite current position?",
...
void OptionsWidget::GetControllerMotors() {
    ...
    if (!result.empty()) {
        ...
        if (args.size() != 3) {
            Message(WARNING,
                    "Could not get positions for reference point " + name,
                    "OptionsWidget::GetControllerMotors");
        } else {
