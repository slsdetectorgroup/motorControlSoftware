...
void OptionsWidget::EnableTube(bool enable) { groupTube->setEnabled(enable); }

void OptionsWidget::EnablePressure(bool enable) {
  groupPressure->setEnabled(enable);
}

void OptionsWidget::LayoutWindow(std::vector<std::string> motors,
                                 std::vector<std::string> refpoints,
                                 std::vector<std::string> fluor,
                                 std::vector<std::string> fwheels) {
...
void OptionsWidget::LoadControllers() {
  std::string result = SendCommand(hostname, 1, "controllerlist ",
                                   "OptionsWidget::LoadControllers");
  if (!result.empty()) {
...
void OptionsWidget::Initialization() {
  ...
  connect(spinPosition, SIGNAL(valueChanged(double)), this,
          SLOT(SetPosition(double)));
  connect(spinUpper, SIGNAL(valueChanged(double)), this,
          SLOT(SetUpperLimit(double)));
  connect(spinLower, SIGNAL(valueChanged(double)), this,
          SLOT(SetLowerLimit(double)));
  connect(comboController, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SetController()));
  connect(pushContSend, SIGNAL(clicked()), this,
          SLOT(SendCommandToController()));
  connect(pushContSendRead, SIGNAL(clicked()), this,
          SLOT(SendandReadCommandToController()));
  connect(comboReferencePoint, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SetRefPoint()));
  ...
  connect(comboFilterWheel, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SetFwheel()));
  connect(pushTubeSend, SIGNAL(clicked()), this, SLOT(SendCommandToTube()));
  connect(pushTubeSendRead, SIGNAL(clicked()), this,
          SLOT(SendandReadCommandToTube()));
}
...
void OptionsWidget::GetMotorController() {
  ...
  std::string result =
      SendCommand(hostname, 2, oss.str(), "OptionsWidget::GetMotorController");
...
void OptionsWidget::GetPosition() {
  ...
  disconnect(spinPosition, SIGNAL(valueChanged(double)), this,
             SLOT(SetPosition(double)));
  std::string result =
      SendCommand(hostname, 2, "pos " + name, "OptionsWidget::GetPosition");
  if (!result.empty()) {
    try {
...
void OptionsWidget::SetPosition(double value) {
  if (Message(QUESTION, "Are you sure you want to overwrite current position?",
              "OptionsWidget::SetPosition") == FAIL) {
    GetPosition();
    return;
  }
  std::string name = std::string(comboMotor->currentText().toAscii().data());
  ...
  std::string result =
      SendCommand(hostname, 3, oss.str(), "OptionsWidget::SetPosition");
  if (!result.empty()) {
    GetPosition();
  }
}
...
void OptionsWidget::SetUpperLimit(double value) {
  ...
  std::string result =
      SendCommand(hostname, 3, oss.str(), "OptionsWidget::SetUpperLimit");
  if (result.empty()) {
    GetUpperLimit();
  }
}
...
void OptionsWidget::SetLowerLimit(double value) {
  ...
  std::string result =
      SendCommand(hostname, 3, oss.str(), "OptionsWidget::SetLowerLimit");
  if (result.empty()) {
    GetLowerLimit();
  }
}
...
void OptionsWidget::GetControllerSerialNumber() {
  ...
  std::string result = SendCommand(hostname, 2, "getcontserialnumber " + name,
  ...
  if (!result.empty()) {
    ...
    std::vector<std::string> args =
        std::vector<std::string>(std::istream_iterator<std::string>(iss),
                                 std::istream_iterator<std::string>());
    if (args.size() != 3) {
      Message(WARNING, "Could not get positions for reference point " + name,
              "OptionsWidget::GetControllerMotors");
    } else {
...
void OptionsWidget::SendCommandToController() {
  ...
  if (args.size() == 0) {
    Message(WARNING, "Command is empty.",
...
void OptionsWidget::SetRefPoint() {
  ...
  std::string result =
      SendCommand(hostname, 2, "refvals " + name, "OptionsWidget::SetRefPoint");
  if (!result.empty()) {
    std::istringstream iss(result);
    std::vector<std::string> args =
        std::vector<std::string>(std::istream_iterator<std::string>(iss),
...
void OptionsWidget::DeleteHolder() {
  if (Message(QUESTION, "Are you sure you want to delete the last holder?",
...
void OptionsWidget::AddHolder() {
  ...
  std::string result = SendCommand(hostname, 2 + 2 * targets.size(), oss.str(),
...
void OptionsWidget::GetTubeInterface() {
  std::string result = SendCommand(hostname, 1, "gettubeinterface ",
                                   "OptionsWidget::GetTubeInterface");
  if (!result.empty()) {
...
void OptionsWidget::SetPressure() { GetPressureInterface(); }

void OptionsWidget::GetPressureInterface() {
  std::string result = SendCommand(hostname, 1, "getpressureinterface ",
                                   "OptionsWidget::GetPressureInterface");
  if (!result.empty()) {
