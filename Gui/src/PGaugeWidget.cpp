  ...
  try {
    ...
    if (result.empty()) {
      ...
      if (startPos == std::string::npos || endPos == std::string::npos ||
          comma == std::string::npos || comma < startPos || comma > endPos) {
        readError = true;
      } else {
        pStatusDisplay->setText(
            pressure.substr(startPos + 1, comma - startPos - 1).c_str());
        pValueDisplay->setText(
            (pressure.substr(comma + 2, endPos - comma - 2) + "  hPa").c_str());
        ...
        if (startPos2 == std::string::npos || endPos2 == std::string::npos ||
          ...
          pStatusDisplay2->setText(
              pressure.substr(startPos2 + 1, comma2 - startPos2 - 1).c_str());
          pValueDisplay2->setText(
              (pressure.substr(comma2 + 2, endPos2 - comma2 - 2) + "  hPa")
                  .c_str());
        }
      }
    }
