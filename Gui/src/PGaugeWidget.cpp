...
void PGaugeWidget::EnableUpdate(bool enable) {
    ...
    if (enable) {
        ...
        if (timer->isActive()) {
            timer->stop();
            Message(WARNING, "Switching off automatic pressure update",
                    "PGaugeWidget::EnableUpdate");
        }
    }
...
void PGaugeWidget::GetPressure() {
    ...
    try {
        std::string result =
            SendCommand(hostname, 1, "pressure ", "PGaugeWidget::GetPressure");
        ...
        if (result.empty()) {
            ...
            if (startPos == std::string::npos || endPos == std::string::npos ||
                comma == std::string::npos || comma < startPos ||
                comma > endPos) {
                readError = true;
            } else {
                pStatusDisplay->setText(
                    pressure.substr(startPos + 1, comma - startPos - 1)
                        .c_str());
                pValueDisplay->setText(
                    (pressure.substr(comma + 2, endPos - comma - 2) + "  hPa")
                        .c_str());
                ...
                if (startPos2 == std::string::npos ||
                    endPos2 == std::string::npos ||
                    comma2 == std::string::npos || comma2 < startPos2 ||
                    comma2 > endPos2) {
                    readError = true;
                } else {
                    pStatusDisplay2->setText(
                        pressure.substr(startPos2 + 1, comma2 - startPos2 - 1)
                            .c_str());
                    pValueDisplay2->setText(
                        (pressure.substr(comma2 + 2, endPos2 - comma2 - 2) +
                         "  hPa")
                            .c_str());
                }
            }
        ...
        disconnect(chkRefresh, SIGNAL(toggled(bool)), this,
                   SLOT(EnableUpdate(bool)));
        chkRefresh->setChecked(false);
        Message(WARNING, "Vacuum pump switched off. ",
                "PGaugeWidget::GetPressure");
        connect(chkRefresh, SIGNAL(toggled(bool)), this,
                SLOT(EnableUpdate(bool)));
