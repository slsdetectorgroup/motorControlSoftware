...
std::string IpToInterfaceName(const std::string &ip) {
    ...
    for (iap = addrs; iap != nullptr; iap = iap->ifa_next) {
        if (iap->ifa_addr && (iap->ifa_flags & IFF_UP) &&
            ...
            sa = reinterpret_cast<sockaddr_in *>(iap->ifa_addr);
            //sa = (struct sockaddr_in *)(iap->ifa_addr);
            inet_ntop(iap->ifa_addr->sa_family, reinterpret_cast<void *>(&(sa->sin_addr)), buf,
...
MacAddr InterfaceNameToMac(const std::string &inf) {
    ...
    for (int j = 0, k = 0; j < 6; j++) {
        k += snprintf(
            ...
            static_cast<int>(static_cast<unsigned int>(static_cast<unsigned char>(ifr.ifr_hwaddr.sa_data[j]))));
