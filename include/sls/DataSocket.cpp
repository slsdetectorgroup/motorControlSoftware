...
int DataSocket::Receive(void *buffer, size_t size) {
    ...
    while (bytes_read < bytes_expected) {
        ...
        if (this_read <= 0)
            break;
        bytes_read += static_cast<int>(this_read);
    }
...
int DataSocket::Send(const void *buffer, size_t size) {
    ...
    while (bytes_sent < (data_size)) {
        ...
        if (this_send <= 0)
            break;
        bytes_sent += static_cast<int>(this_send);
    }
...
int DataSocket::write(void *buffer, size_t size) {
    return static_cast<int>(::write(getSocketId(), buffer, size));
}

int DataSocket::read(void *buffer, size_t size) {
    return static_cast<int>(::read(getSocketId(), buffer, size));
}
...
void DataSocket::close() {
    if (sockfd_ > 0) {
        if (::close(sockfd_)) {
            throw RuntimeError("could not close socket");
        }
        sockfd_ = -1;
    } else {
        throw RuntimeError("Socket ERROR: close called on bad socket\n");
    }
}
