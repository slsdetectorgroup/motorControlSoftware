...
// O_NOCTTY flag tells UNIX that this program doesn't want to be the
// "controlling terminal" for that port. O_NDELAY flag tells UNIX that this
// program doesn't care what state the DCD signal line is in - whether the other
// end of the port is up and running. If you do not specify this flag, your
...
Interface::Interface(std::string serial, int serialPortNumber,
...
Interface::~Interface() { close(serialfd); }

std::string Interface::getSerial() { return serial; }

int Interface::getSerialPortNumber() { return serialPortNumber; }
...
void Interface::ValidateController() {
  ...
  try {
    ...
    if (result.find(std::string(CONTROLLER_MODEL_RESPONSE)) ==
        std::string::npos) {
...
std::string Interface::ControllerSendCommand(std::string command,
                                             bool readBack) {
  ...
  while (ret == -1) {
    ...
    if (attempt == CONTROLLER_MAX_TX_ATTEMPTS) {
      std::ostringstream oss;
      oss << "Send attempt number " << CONTROLLER_MAX_TX_ATTEMPTS << " for ["
          << buffer << "] to port " << serial << ". Aborting command.";
      if (verbose) {
  ...
  while (ret == -1) {
    ...
    if (attempt == CONTROLLER_MAX_RX_ATTEMPTS) {
      std::ostringstream oss;
      oss << "Receive attempt number " << CONTROLLER_MAX_RX_ATTEMPTS << " for ["
          << buffer << "] to port " << serial << ". Aborting read.";
      if (verbose) {
        FILE_LOG(logERROR) << oss.str();
      }
      throw std::runtime_error(oss.str());
...
std::string Interface::PressureGaugeSendCommand(std::string command) {
  ...
  if (strncmp(result, PRESSURE_ACK_RESPONSE, strlen(PRESSURE_ACK_RESPONSE))) {
    ...
    if (!strncmp(result, PRESSURE_NACK_RESPONSE,
                 strlen(PRESSURE_NACK_RESPONSE))) {
  ...
  while (ret == -1 || strlen(result) < 5) {
    ...
    if (attempt == PRESSURE_MAX_RX_ATTEMPTS) {
      std::ostringstream oss;
      oss << "Receive attempt number " << PRESSURE_MAX_RX_ATTEMPTS << " for ["
          << buffer << "] to pressure gauge " << serial << ". Aborting read.";
      if (verbose) {
        FILE_LOG(logERROR) << oss.str();
      }
      throw std::runtime_error(oss.str());
...
void Interface::ValidateFilterWheel() {
  ...
  try {
    std::string result =
        FilterWheelSendCommand(FILTER_WHEEL_IDENTIFY_CMD, true);
    if (result.find(std::string(FILTER_WHEEL_MODEL_RESPONSE)) ==
        std::string::npos) {
...
std::string Interface::FilterWheelSendCommand(std::string command,
  ...
  while (ret == -1) {
    ...
    if (attempt == FILTER_WHEEL_MAX_RX_ATTEMPTS) {
      std::ostringstream oss;
      oss << "Receive attempt number " << FILTER_WHEEL_MAX_RX_ATTEMPTS
  ...
  // extract result
  // if segfault here, it needs more time to read (increase
  // FILTER_WHEEL_READ_WAIT_US)
