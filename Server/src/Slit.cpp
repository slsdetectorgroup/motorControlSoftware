...
Slit::Slit(Controller* controllerX1, Controller* controllerX2, int axisX1, int axisX2) 
    : controllerX1(controllerX1), controllerX2(controllerX2), axisX1(axisX1), axisX2(axisX2),limit(SLITS_SUM_LIMIT) {
...
void Slit::updateLimits() {
    ...
    FILE_LOG(logINFO) << "Updated upperlimits of slits (" << controllerX1->getMotor(axisX1)->getUpperLimit()
...
void Slit::setSlitWidth(double width) {
    ...
    if (!Motor::matches(getSlitWidth(), width)) {
        std::ostringstream oss;
        oss << "Could not set slit width to " << width << ". Got " << std::fixed << getSlitWidth();
        throw RuntimeError (oss.str());
    }
}
...
void Slit::setCenter(double center) {
    ...
    // move the appropriate one first, else they will crash (update limits each time else they will not move)
    if (relPos > 0) {
        controllerX2->moveAbs(x2NewPos, axisX2);
        updateLimits();
        controllerX1->moveAbs(x1NewPos, axisX1);
    ...
    if (!Motor::matches(getCenter(), center)) {
        std::ostringstream oss;
        oss << "Could not set center to " << center << ". Got " << std::fixed << getCenter();
        throw RuntimeError (oss.str());
    }
}
