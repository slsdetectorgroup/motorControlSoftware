  ...
  */
  try {
    ...
    if (!strcasecmp(command.c_str(), "sendcontroller")) {
      if (nArg < 3) {
        throw RuntimeError(
            "Requires 3 parameters: sendcontroller [corvus] [commands..]");
      }
      string name = args[1];
      ...
      if (nArg < 3) {
        throw RuntimeError(
            "Requires 3 parameters: readcontroller [corvus] [commands..]");
      }
      string name = args[1];
      int icontroller = GetControllerIndex(name);
      string advancedCommand;
    ...
    // ----- motor --------------------------------------------------------
      ...
      iss >> value;
      if (iss.fail()) {
        throw RuntimeError("Could not scan relative position argument " +
                           args[2]);
      }
      ...
      iss >> value;
      if (iss.fail()) {
        throw RuntimeError("Could not scan absolute position argument " +
                           args[2]);
      }
    ...
    // ----- flourescence -------------------------------------------------
      ...
      if (nArg != 3) {
        throw RuntimeError(
            "Requires 3 parameters: setholder [fluorescence motor] [index]");
      }
      string name = args[1];
      OnlyFluorescenceCommand(name);
      int ifluor = GetFluorescenceIndex(name);
      ...
      if (nArg != 2) {
        throw RuntimeError(
            "Requires 2 parameters: dellastholder [fluorescence motor]");
      }
      string name = args[1];
      OnlyFluorescenceCommand(name);
      int ifluor = GetFluorescenceIndex(name);
...
#ifdef XRAYBOX
        throw RuntimeError(
            "Requires 18 parameters: addholder [fluorescence motor] [Target 8] "
            "[Energy 8] ..[Target 1] [Energy 1]");

#else
        throw RuntimeError(
            "Requires 18 parameters: addholder [fluorescence motor] [Target 1] "
      ...
      if (nArg != 3) {
        throw RuntimeError("Requires 3 parameters: fllist [fluorescence motor] "
                           "[holder index]");
      }
      string name = args[1];
      OnlyFluorescenceCommand(name);
      ...
      if (nArg != 3) {
        throw RuntimeError(
            "Requires 3 parameters: setfl [fluorescence motor] [target name]");
      }
      string name = args[1];
      OnlyFluorescenceCommand(name);
      int ifluor = GetFluorescenceIndex(name);
    ...
    if (!strcasecmp(command.c_str(), "sendtube")) {
      ...
      if (result != value) {
        oss << "Could not set voltage to " << value << " kV. Voltage is at "
            << result << " kV.";
        throw RuntimeError(oss.str());
      }
      oss << result << " kV";
      ...
      if (result != value) {
        oss << "Could not set current to " << value << " mA. Current is at "
            << result << " mA.";
        throw RuntimeError(oss.str());
      }
      oss << result << " mA";
      ...
      if (result.first != voltage) {
        oss << "Could not set voltage to " << voltage << " kV. Voltage is at "
            << result.first << " kV.";
        throw RuntimeError(oss.str());
      }
      if (result.second != current) {
        oss << "Could not set current to " << current << " mA. Current is at "
            << result.second << " mA.";
        throw RuntimeError(oss.str());
      }
    ...
    // ----- filter wheels -------------------------------------------------
      ...
      iss >> value;
      if (iss.fail()) {
        throw RuntimeError("Could not scan filter wheel value argument " +
                           args[2]);
      }
  ...
  // delete xray class if tube switched off
    ...
    if (xrayTube != NULL) {
      ...
      FILE_LOG(logINFO) << "Setting interface /ttyUSB" << port
                        << ": Not in use";
      delete xrayTube;
      xrayTube = NULL;
    }
  ...
  for (unsigned int i = 0; i < usbSerialPortsUsed.size(); ++i) {
    ...
    try {
      ...
      case CONTROLLER:
        ...
        break;
      case FILTER_WHEEL:
        if (Fwheel::CheckFWSerialNumber(i,
                                        fwheel[lastIndex]->getSerialNumber())) {
          usbSerialPortsUsed[i] = true;
...
void Initialize::ReferencePointMode(vector<string> args) {
  ...
  if (referencePoints == NULL) {
    ...
    try {
      ...
      throw RuntimeError("Could not add reference point. Missing Detector_x, "
...
void Initialize::FluorescenceMode(vector<string> args) {
  if (args.size() != CONFIG_FLUOR_NUM_ARGS) {
    ostringstream oss;
    oss << "Incorrect number of arguments for fluorescence/fluorescence_wheel. "
           "Expected "
...
void Initialize::ControllerMode(vector<string> args) {
  ...
  for (unsigned int i = 0; i < controller.size(); ++i) {
    if (name == controller[i]->getName() ||
      ...
      throw RuntimeError("Two controllers cannot have the same name " + name +
                         " or serial number " + serialNumber);
    }
  }
...
void Initialize::MotorMode(vector<string> args) {
  ...
  double lowerLimit = -1;
  {
    ...
    iss >> lowerLimit;
    if (iss.fail()) {
      throw RuntimeError("Could not scan lowerLimit for motor argument: " +
                         args[4]);
    }
  }
  double upperLimit = -1;
  {
    ...
    iss >> upperLimit;
    if (iss.fail()) {
      throw RuntimeError("Could not scan upperLimit for motor argument: " +
                         args[5]);
    }
  }
  ...
  // create motor object
  motor.push_back(
      new Motor(motor.size(), name, axis, icontroller, lowerLimit, upperLimit));
