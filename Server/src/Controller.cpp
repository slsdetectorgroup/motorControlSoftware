...
Controller::Controller(const int index, const std::string name,
  ...
  FILE_LOG(logINFO) << "Contoller  [" << index << "]: [name:" << name
...
void Controller::setInterface(Interface *interface) {
  ...
  FILE_LOG(logINFO) << "\tController " << name
...
void Controller::setMotor(const int axis, Motor *motor) {
  ...
  if (axis < 0 || axis >= MAX_NUM_MOTORS_PER_CONTROLLER) {
    ...
    oss << "Invalid axis index " << axis << " to set motor " << motor->getName()
        << " for controller " + name;
...
void Controller::moveRel(double position, int axis) {
  ...
  if (!motor[axis]->canMotorMove(currentPosition + position)) {
    oss << "Cannot move " << motor[axis]->getName() << " by " << position
        << " (to position " << currentPosition + position << "). Beyond limits "
        << motor[axis]->getLowerLimit() << " and "
...
#endif
  FILE_LOG(logINFO) << "Sending command to read back: [" << command << "] to "
                    << name;
...
void Controller::print() {
  std::cout << "\tController [" << index << "]\n"
            ...
            << "\t  Motors 0     : "
            << ((motor[0] == NULL) ? "None" : motor[0]->getName()) << std::endl
            << "\t  Motors 1     : "
            << ((motor[1] == NULL) ? "None" : motor[1]->getName()) << std::endl
            << "\t  Motors 2     : "
            << ((motor[2] == NULL) ? "None" : motor[2]->getName()) << std::endl
            << std::endl;
}
